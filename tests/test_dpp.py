...
import numpy as np
import pytest
from dccp.linearize import linearize, LinearizationData

def test_linearize_dpp_basic():
...
def test_benchmark_dpp_vs_rebuild():
    ...
    for _ in range(iterations):
        ...
        try:
            ...
            pass # Solver might not be available or other issues
    end_time = time.time()
    ...
    print(f"\nRebuild time (incl. canonicalization): {rebuild_time:.4f}s")
    print(f"Update time (incl. canonicalization):  {update_time:.4f}s")
    print(f"Speedup:      {rebuild_time / update_time:.2f}x")
